...
const singularIrregulars = Object.fromEntries(
  Object.entries(irregulars).map(([singular, plural]) => [plural, singular]),
);
 ...
 * suffix (s, x, z, ch, sh).
 */
function takesEsSuffix(word: string): boolean {
  return (
    word.endsWith("s") ||
    word.endsWith("x") ||
    word.endsWith("z") ||
 ...
 */
function matchCase(source: string, inflected: string): string {
 ...
 */
export function detectPlural(word: string): boolean {
  if (!word) return false;
  const lower = word.toLowerCase();

  if (uncountables.has(lower)) return false;
  if (irregulars[lower]) return false;
  if (singularIrregulars[lower]) return true;

  if (lower.endsWith("ies") && lower.length > 3) return true;
  if (lower.endsWith("es") && takesEsSuffix(lower.slice(0, -2))) return true;
  if (
    lower.endsWith("s") &&
    !lower.endsWith("ss") &&
    !lower.endsWith("us") &&
...
export function singularize(word: string): string {
  ...
  if (singular) return matchCase(word, singular);

  if (!detectPlural(word)) return word;
...
const comparativeIrregulars: Record<string, string> = {
  good: "better",
  bad: "worse",
  far: "farther",
  little: "less",
...
const superlativeIrregulars: Record<string, string> = {
  good: "best",
  bad: "worst",
  far: "farthest",
  little: "least",
...
function shouldDouble(word: string): boolean {
  ...
  if (!/[bcdfghjklmnpqrstvz]/.test(last)) return false;

  // Must have single vowel before the consonant
  if (!/[aeiou]/.test(secondLast)) return false;
